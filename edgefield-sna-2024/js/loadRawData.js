...
$(document).ready(function() {
	...
	$(".psleToggle").click(togglePSLEData);
	...
	for (let n of sheets)
	{
		switch (n.toLocaleUpperCase().trim())
		{
			...
			case "DATA - STREAM":
				...
				break;
			case "DATA - PSLE":
				processAdditionalData(arr[n], "PSLE");
				break;
			case "DATA - CLASSLIST":
...
function showGraph(e)
{
	...
	if (Object.keys(studentData).length !== 0 && Object.keys(classOutputData).length !== 0) {
		...
		if (absentStudents.length > 0) {
			...
			let str = '<strong>List of students who did not complete the SNA survey:</strong><ul>';
			...
			let str = '<strong>All students from class ' + uploadedClass + ' completed the SNA survey.</strong>';
			$('#absenceList').html(str);
...
function initSheetJSEvents()
{
	...
	/*$("#dropAreaDrop").on("dragenter", handleExcelDropDragover);
	$("#dropAreaDrop").on("dragover", handleExcelDropDragover);
	$("#dropAreaDrop").on("drop", handleExcelFile);*/
